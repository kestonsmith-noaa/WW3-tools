...
fl="RTopo_2_0_4_GEBCO_v2023_60sec_pixel.CRMformat.nc"
fl="crm_vol10_2023.nc.S250m.nc"
...
xg, yg, eg = FE.loadWW3MeshCoords(mesh)
np.savetxt("e0.txt", eg,fmt='%i')
np.savetxt("xyg0.txt", np.vstack((xg.T,yg.T)).T )
j=np.where(np.sum(eg,1)>0)
print("good ele list")
print(j)
eg=eg[j,:]
eg=eg[0,:,:]

xg, yg, zg, eg, bndg =FE.loadWW3Mesh(mesh)
#print(eg.shape)
#np.savetxt("xyg1.txt", np.vstack((xg.T,yg.T)).T )
#np.savetxt("e1.txt", eg, fmt='%i')
#FE.WriteWW3Mesh("Mesh1.WW3.msh",xg,yg,zg,eg,bndg)
nex=eg.shape[0]
eg=np.vstack((eg,[0,0,0]))
...
xig=[]
yig=[]
WI=np.array([float("nan"),float("nan"),float("nan")])
NI=np.array([0,0,0])
F=np.array([float("nan")])
X=np.array([float("nan")])
Y=np.array([float("nan")])
#EI=np.array([float("nan")])

MaxDist=10.**10
for kx in range(nxl):
    if (x[kx]>min(xg) and x[kx]<max(xg)):
            jyl=np.where( y > min(yg))
            jyh=np.where( y < max(yg))
            jy=np.intersect1d(jyh,jyl)
            print(jy)
#            jy=np.where( y > min(yg) and y <max(yg)   )
            wi,ni,ei,di=mshint.compute_mesh_to_mesh_interp_weights(xg, yg, eg, x[kx]+0.*y[jy], y[jy], flout, MaxDist)
                
            #F=np.hstack((F,z[kx,jy]))
            X=np.hstack((X,x[kx]+0.*y[jy]))
            Y=np.hstack((Y,y[jy]))
            F=np.hstack((F,z[jy,kx]))
            WI=np.vstack((WI,wi))
            NI=np.vstack((NI,ni))
#            EI=np.hstack((EI,ei))
print(WI)
print(NI)
#print(EI)
np.savetxt('weights.txt', WI)
np.savetxt('nodes.txt', NI,fmt='%d')
np.savetxt('values.txt', np.transpose(np.vstack((X,Y, F))) )


Avalues=np.array([float("nan")])
row_ind=Avalues
col_ind=row_ind
f=np.array([float("nan")])
nd=WI.shape[0]
...
for k in range(nd):
    #print(WI[k,:])
    if np.sum(WI[k,:])>.99:
        Avalues=np.hstack((Avalues,WI[k,:]))
        row_ind=np.hstack((row_ind,[n,n,n]))
        col_ind=np.hstack((col_ind,NI[k,:]))
        f=np.hstack((f,F[k]))
        n=n+1
...
print("n="+str(n))
print("nn="+str(nn))
print("max row"+str(np.max(row_ind)))
print("max col"+str(np.max(col_ind)))
print("min row"+str(np.min(row_ind)))
print("min col"+str(np.min(col_ind)))

A=csr_matrix((Avalues, (row_ind, col_ind-1)), shape=(n, nn)).toarray()
print(A)

Q=np.transpose(A) @ A
b=-2*np.transpose(A) @ f 
print(A)
print(Q)
print(b)
print(f)
...
#def gradient_function(x, Q, b):
def gradient_function(x):
    return 2.*Q @ x + b

#def hessian_product(x, p, Q):
def hessian_product(p, x_ignored):
    # x is the current optimization vector (not strictly needed for a constant H)
...
np.savetxt('xhat.txt', res.x )

dfdx,dfdy=FE.GradientOnElements(xg, yg, res.x, eg)
np.savetxt('dfdx.txt',dfdx )
np.savetxt('dfdy.txt',dfdy )
#A=FE.ElementArea(xg, yg, eg)

eg=eg[0:nex,:]
#Now Add Smoothing constraint
#<dhdx^2+dhdy^2>
ddx, ddy = FE.GradientMatrixToEle(xg, yg, eg)

dpsidx=ddx @ res.x
dpsidy=ddy @ res.x
np.savetxt('dpsidx.txt',dpsidx  )
np.savetxt('dpsidy.txt',dpsidy  )

dxdx=ddx @ xg
dxdy=ddy @ xg
np.savetxt('dxdx.txt',dxdx  )
np.savetxt('dxdy.txt',dxdy  )

ddxN,ddyN=FE.GradientMatrixToNode(xg, yg, eg)
dfdx=ddxN @ res.x
dfdy=ddyN @ res.x
np.savetxt('dfdxN.txt',dfdx  )
np.savetxt('dfdyN.txt',dfdy  )

dpsidx=ddx @ res.x
dpsidy=ddy @ res.x
np.savetxt('dhdx.txt',dpsidx  )
np.savetxt('dhdy.txt',dpsidy  )
...
EleAr=FE.ElementAreaP(xg, yg, eg)
ne=len(EleAr)
AD = diags(EleAr)
#AD = diags(np.sqrt(AA))
#AD = diags(A, k=0)
...
Q=np.transpose(A) @ A + beta*( Qx + Qy )
#Q=np.transpose(A) @ A + beta*( Qx @ Qx + Qy @ Qy )

dpsid2=( Qx @ Qx + Qy @ Qy ) @ res.x
np.savetxt('dpsid2.txt',dpsid2  )
...
print(f"Optimization result: {res.x}")
np.savetxt('xhatSmooth.txt', res.x )
dpsidx=ddx @ res.x
dpsidy=ddy @ res.x
np.savetxt('dhsdx.txt',dpsidx  )
np.savetxt('dhsdy.txt',dpsidy  )


beta=100.
ddx,ddy=FE.GradientMatrixToNode(xg, yg, eg)
Qx = ddx.T @ ddx
Qy = ddy.T @ ddy
Qx = ddx @ ddx
Qy = ddy @ ddy
...
print(f"Optimization result: {res.x}")
np.savetxt('xhatSmooth2.txt', res.x )
...
"""
            ...
            zi=data["z"][jy,kx]
            zig.append(np.array(zi).flatten())
            yig.append(np.array(y[jy]).flatten())
            xig.append(np.array(x[kx]+0.*y[jy]).flatten())

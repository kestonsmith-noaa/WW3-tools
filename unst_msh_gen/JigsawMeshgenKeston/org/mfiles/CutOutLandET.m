function omesh = CutOutLandET(mesh,S,topo)
...
lonT=topo.point.coord{:,1};
latT=topo.point.coord{:,2};
V=topo.value;
Vp=[V(:,end),V,V(:,1)];

dlon=mean(lonT(2:end)-lonT(1:end-1))
lonTp=[lonT(1)-dlon,lonT',lonT(end)+dlon]';

tic
%ze=interp2(topo.point.coord{:,1},topo.point.coord{:,2},topo.value,xe,ye);
ze=interp2(lonTp,latT,Vp,xe,ye);
toc
...
jo=find(ze<=oclimit);
jl=find(ze>lnlimit);
(length(jo)+length(jl))/ne
jsl=setdiff(1:ne,union(jo,jl));
...
for k=1:N
    if mod(k,100)==0,
        ...
        disp(['remaining unclassed: ', int2str( length(jsl) )]);
    ...
    if length(xl)>2,
        ...
        jbox=intersect(jbox,jsl);% don't care about nodes already clased

...
def load_configuration(config_path):
    ...
    configurations = {
        'input_mesh': config.get('MeshSettings', 'input_mesh', fallback=''),
...
if (__name__ == "__main__"):
    ...
    mesh_file = configurations['input_mesh']
    
    print("working with input global mesh " + mesh_file )
    print( len(mesh_file) )
    n=len(mesh_file) 
    print( mesh_file[0:n-4])
    
    jigsawpy.jigsaw_msh_t()
    jigsawpy.loadmsh(mesh_file, mesh)
    ...
    flout=mesh_file[0:n-4]+"FltOcn.msh"
    print("saving ocean mesh to: " + flout )
    jigsawpy.savemsh(flout, mesh)
    
# convert to lon lat    
    point = mesh.point["coord"]
    point = jigsawpy.R3toS2(geom.radii, point) 
    point*= 180. / np.pi
    mesh.point["coord"][:,0]=point[:,0]
    mesh.point["coord"][:,1]=point[:,1]
    
    flout=mesh_file[0:n-4]+"FltOcnLL.msh"
    print("saving lat,lon mesh to: " + flout )
    jigsawpy.savemsh(flout, mesh)
 #   jigsawpy.savemsh("mesh_1kmUSCL_15kmOO_FltOcnLL.msh", mesh)
    interp_bathymetry()
    #interpolate bathymetry to nodes and add to 3rd column of points
    flout=mesh_file[0:n-4]+"FltOcnLLH.msh"
    print("saving lat,lon and bathymetry mesh to: " + flout )
    jigsawpy.savemsh(flout, mesh)
    
    flout=flout[0:n-4]+".ww3"
    cells = [("triangle", mesh.tria3["index"])]
    tri_data=cells[0][1]+1
    pointx = mesh.point["coord"]
    pointx[:,0]=mesh.point["coord"][:,0]
    pointx[:,1]=mesh.point["coord"][:,1]
    pointx[:,2]=mesh.point["coord"][:,2]
    write_gmsh_mesh(flout, pointx, tri_data)
   
    # viz. in eg. paraview
    flout=flout[0:n-4]+".vtk"
    print("saving .vtk file for paraview to: " + flout )
    jigsawpy.savevtk(flout, mesh)

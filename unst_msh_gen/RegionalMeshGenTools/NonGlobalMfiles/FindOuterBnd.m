function OpenBndNodes=FindOuterBnd(meshflin,pslgflin)
...
bnd=detbndy(e);%find boundary elements(line segments)
c=edges2chains(bnd);%find closed chains in boundary elements(these are islands or )
for k=1:length(c)%compute area of each closed chain
    k
...
title('red circles should be open ocean boundary nodes')
OpenBndNodes=nooext;
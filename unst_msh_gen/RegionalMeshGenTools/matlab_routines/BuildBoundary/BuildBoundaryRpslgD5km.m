...
for k=1:N
    ...
    if length(S(k).X)>3,
        x=S(k).X(1:end-1);
        y=S(k).Y(1:end-1);
        %S(k).area=R*R*areaint(y,x)/10^6;
        S(k).area=areaint(y,x,earth);
    else
        S(k).area=0;
    end
    A(k)=S(k).area;
...
for k=1:N
    ...
    if and(length(x)>minedeges,S(k).area > minarea)
...
%make outer bound
n=length(pslg.x);
pslg.x=[pslg.x,Bx(2:4)];%add northwest, South east, and north east box corner nodes
pslg.y=[pslg.y,By(2:4)];

nc=length(pslg.chains);%add south west,northwest and north east box corner chains
...
if isplot==1
    ...
    figure;drawpslg_chainsXnl(pslgb);title('PSLG with all points interior to final outer boundary')
end
...
save PSLGboundary5km_shifted.mat pslg

geom=pslg2geom(pslg)

savemsh('PSLGboundary5km_shifted.msh',geom)
